...
import { Send, Users, Loader2, Building2, Mail, Copy, Check } from "lucide-react";
...
export default function SCSendLinksPage() {
  ...
  const copyMessageTemplate = async (student: Student) => {
    const hash = hashMap[student.uid];
    if (!hash) {
      alert("Link not ready yet. Please wait.");
      return;
    }
...
Apply here: ${formLink}`;
    ...
    try {
      ...
      setCopiedUid(student.uid);
      setTimeout(() => setCopiedUid(null), 1000);
    } catch (err) {
                    ...
                    {filteredStudents.map((student) => (
                      ...
                      >
                        ...
                        <td className="p-4 text-center">
                          <button
                            onClick={() => copyMessageTemplate(student)}
                            disabled={preparingLinks}
                            className="inline-flex items-center gap-2 bg-gray-600 hover:bg-gray-700 text-white px-5 py-2 rounded-lg font-bold transition-all disabled:opacity-30 disabled:grayscale"
                          >
                            {copiedUid === student.uid ? (
                              <>
                                <Check size={16} />
                                Copied
                              </>
